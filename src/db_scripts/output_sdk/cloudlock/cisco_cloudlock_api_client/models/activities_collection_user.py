from collections.abc import Mapping
from typing import Any, TypeVar, Union

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

T = TypeVar("T", bound="ActivitiesCollectionUser")


@_attrs_define
class ActivitiesCollectionUser:
    """The user information.

    Attributes:
        user_email (Union[Unset, str]):
        vendor_id (Union[Unset, str]):
    """

    user_email: Union[Unset, str] = UNSET
    vendor_id: Union[Unset, str] = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        user_email = self.user_email

        vendor_id = self.vendor_id

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({})
        if user_email is not UNSET:
            field_dict["user_email"] = user_email
        if vendor_id is not UNSET:
            field_dict["vendor_id"] = vendor_id

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        user_email = d.pop("user_email", UNSET)

        vendor_id = d.pop("vendor_id", UNSET)

        activities_collection_user = cls(
            user_email=user_email,
            vendor_id=vendor_id,
        )

        activities_collection_user.additional_properties = d
        return activities_collection_user

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
