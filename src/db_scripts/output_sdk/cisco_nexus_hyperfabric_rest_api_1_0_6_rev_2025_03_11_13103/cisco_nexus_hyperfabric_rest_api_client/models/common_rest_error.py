...
class CommonRestError:
    """A REST API Error is returned when the Hyperfabric service failed to execute the received REST API request to inform
    the user on the type of the error and other useful information for troubleshooting the request.

        Attributes:
            causes (Union[Unset, list[str]]): A list of underlying causes.
            critical (Union[Unset, bool]): The flag that indicates if the error is a critical error or not.
            err_code (Union[Unset, CommonErrCode]): An error code provides additional information about a failed API
                request.
            field (Union[Unset, str]): The name of the relevant field.
            message (Union[Unset, str]): The error message.
            notes (Union[Unset, str]): Additional information about the error.
            status (Union[Unset, int]): The HTTP status code associated with the error.
            tracking_id (Union[Unset, str]): A unique identifier for each request, used for tracking purposes.
            value (Union[Unset, str]): The value of the relevant field.
    """
