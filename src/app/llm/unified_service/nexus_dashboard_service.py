# app/llm/unified_service/nexus_dashboard_service.py

# Auto-generated – DO NOT EDIT
from app.llm.platform_clients.nexus_dashboard_client import Nexus_dashboardClient

class Nexus_dashboardServiceClient:
    """Generic call-through service used by FastAPI."""

    def __init__(self, **sdk_kwargs):
        self.client = Nexus_dashboardClient(**sdk_kwargs)

    def call(self, function_name: str, **kwargs):
        try:
            method = getattr(self.client, function_name)
        except AttributeError:
            raise ValueError(
                f"No such method '{function_name}' on Nexus_dashboardClient"
            )
        return method(**kwargs)