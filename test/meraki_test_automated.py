...
# Test queries
TEST_QUERIES = [
    "get meraki organization alerts profile",
    "get meraki device list",
    "list meraki inventory",
    "get meraki policy objects",
    "get meraki org admins",
    "get meraki organization"
]

class AutomatedMerakiTester:
    def __init__(self, port: int = 8003):  # Use 8003 to avoid conflicts
        self.port = port
    ...
    def _detect_function(self, query: str, response: str) -> str:
        ...
        if "alerts" in query_lower or "alert" in query_lower:
            return "getOrganizationAlertsProfiles"
        elif "device" in query_lower and "list" in query_lower:
            return "getOrganizationDevices"
        elif "inventory" in query_lower:
            return "getOrganizationInventoryDevices"
        elif "policy" in query_lower and "object" in query_lower:
            return "getOrganizationPolicyObjects"
        elif "admin" in query_lower:
            return "getOrganizationAdmins"
    ...
    def run_all_tests(self):
        """Run all tests"""
        print("\n" + "="*60)
        print("Running Meraki Test Suite")
        print("="*60)
    ...
    def generate_report(self) -> bool:
        ...
        report_file = REPO_ROOT / "test" / f"meraki_automated_results_{timestamp}.json"
        ...
        report_data = {
            ...
            "environment": {
                "CISCO_MERAKI_API_KEY": "Set" if os.getenv("CISCO_MERAKI_API_KEY") else "Not set",
                "MERAKI_ORG_ID": "Set" if os.getenv("MERAKI_ORG_ID") else "Not set"
            },
    ...
    def run(self) -> bool:
        """Main execution flow"""
        print(f"🤖 Meraki Automated Test Suite")
...
def main():
    ...
    # Check environment
    required_vars = ["CISCO_MERAKI_API_KEY"]
    optional_vars = ["MERAKI_ORG_ID"]
    ...
    if missing_optional:
        print("ℹ️  INFO: Missing optional environment variables:")
        for var in missing_optional:
            print(f"   - {var}")
        print("Some tests may use defaults.\n")
    
    # Run automated test
    tester = AutomatedMerakiTester(port=port)
    success = tester.run()
